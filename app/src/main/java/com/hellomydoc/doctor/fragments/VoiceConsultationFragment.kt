...
import com.hellomydoc.doctor.databinding.FragmentVoiceConsultationBinding
import kotlinx.coroutines.launch

class VoiceConsultationFragment : Fragment() {
    private var childCallback: HomeActivity.ChildCallback? = null

    companion object {
        @JvmStatic
        fun newInstance(ChildCallback: HomeActivity.ChildCallback) = VoiceConsultationFragment().apply {
            childCallback = ChildCallback
        }
    }
    private var appointments: List<AppointmentData> = listOf()
    private lateinit var binding: FragmentVoiceConsultationBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentVoiceConsultationBinding.inflate(layoutInflater)
        return binding.root
    }
    ...
    private fun setupUpcomingAppointments() {
        ...
        lifecycleScope.launch {
            try {
                val response = repository.getVoiceUpcomingAppointments(repository.userUid).resp
                when{
