...
class ChatConsultationFragment : Fragment() {
    ...
    companion object {
        @JvmStatic
        fun newInstance(ChildCallback: HomeActivity.ChildCallback) = ChatConsultationFragment().apply {
            childCallback = ChildCallback
        }
    }
    private var appointments: List<AppointmentData> = listOf()
    private lateinit var binding: FragmentChatConsultationsBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentChatConsultationsBinding.inflate(layoutInflater)
        return binding.root
    }
    ...
    private fun setupUpcomingAppointments() {
        ...
        lifecycleScope.launch {
            try {
                val response = repository.getChatUpcomingAppointments(repository.userUid).resp
                when{
